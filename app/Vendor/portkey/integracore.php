...
function integracoreGetExpectedReceipts($params=array()){
	...
	//return $post;
	$rtn=$post['xml_array']['soapEnvelope']['soapBody']['getExpectedReceiptsResponse']['getExpectedReceiptsResult'];
	return $rtn;
}
//---------------------------
function integracoreAddExpectedReceipts($params=array()){
	...
	$post=postXML($url,$xmlpost,array('-ssl'=>false));
	//return $post;
	$rtn=$post['xml_array']['soapEnvelope']['soapBody']['addExpectedReceiptsResponse']['addExpectedReceiptsResult'];
	return $rtn;
}
...
function integracoreGetShipConfirms($params=array()){
	...
	$xmlpost .= '</soap12:Envelope>'."\n";
	$post=postXML($url,$xmlpost,array('-ssl'=>false));
	$orders=$post['xml_array']['soapEnvelope']['soapBody']['getShipConfirmsResponse']['getShipConfirmsResult']['OrderStatus'];
	return $orders;
}
//---------------------------
function integracoreGetMultipleOrderStatus($params=array()){
	...
	if(!isset($post['xml_array']['soapEnvelope']['soapBody']['getMultipleOrderStatusResponse']['getMultipleOrderStatusResult'])){
		return $post;
	}
	$result=$post['xml_array']['soapEnvelope']['soapBody']['getMultipleOrderStatusResponse']['getMultipleOrderStatusResult'];
	if(isset($result['Order']) && is_array($result['Order'])){return $result['Order'];}
	return $post;
}
//---------------------------
function integracoreResetOrderStatus($params=array()){
	...
	$post=postXML($url,$xmlpost,array('-ssl'=>false));
	//return $post;
	$orders=$post['xml_array']['soapEnvelope']['soapBody']['resetOrderNumbersResponse']['resetOrderNumbersResult']['OrderStatus'];
	return $orders;
}
...
function integracoreGetItems($params=array()){
	...
	$post=postXML($url,$xmlpost,array('-ssl'=>false));
	//return $post;
	$items=$post['xml_array']['soapEnvelope']['soapBody']['getItemsResponse']['getItemsResult']['Item'];
	return $items;
}
//---------------------------
function integracoreAddItems($params=array()){
	...
	$post=postXML($url,$xmlpost,array('-ssl'=>false));
	//return $post;
	$items=$post['xml_array']['soapEnvelope']['soapBody']['addItemsResponse']['addItemsResult']['Item'];
	return $items;
}
//---------------------------
function integracoreSetItems($params=array()){
	...
	$post=postXML($url,$xmlpost,array('-ssl'=>false));
	//return $post;
	$items=$post['xml_array']['soapEnvelope']['soapBody']['setItemsResponse']['setItemsResult']['Item'];
	return $items;
}
//---------------------------
...
function integracoreGetInventory($params=array()){
	...
	$post=postXML($url,$xmlpost,array('-ssl'=>false));
	$xml = soap2XML($post['body']);
	$info=array('items'=>array());
	$fields=array();
	foreach($xml->soapBody->GetCompleteInventoryResponse->GetCompleteInventoryResult->InventoryList->ItemInventory as $item){
		$sku=(string)$item->PartNumber;
		$crec=array();
		foreach($item as $citem=>$val){
			$key=(string)$citem;
			if(!preg_match('/^(SiteId|PartNumber)$/i',$key)){$fields[$key]+=1;}
			if(isNum((string)$val)){$info['items'][$sku][$key]=(real)$val;}
			else{$info['items'][$sku][$key]=removeCdata((string)$val);}
			}
    	}
...
function integracoreGetOrderStatus($params=array()){
	...
	//echo "integracoreGetOrderStatus:" . printValue($post);
 	$arr=$post['xml_array'];
 	$orders=$arr['soapEnvelope']['soapBody']['GetMultipleOrderStatusResponse']['GetMultipleOrderStatusResult']['OrderStatusList']['OrderStatus'];
...
//-------------------------------
function integracorePostOrders($orders=array()){
	//info: returns the order status for ordernumbers
	//info: this function requires user and pass and an array of ordernumbers.
	//usage: $status=integracoreGetOrderStatus(array('user'=>"username",'pass'=>"password",'ordernumbers'=>array('ON12343','ON34323','ON752345')));
	...
	$results['xml_array']=$arr;
 	$recs=$arr['soapEnvelope']['soapBody']['OrderImportResponse']['OrderImportResult']['OrderMessage']['OrderResult'];
 	if(!is_array($recs)){
		$results['_status']='FAILED';
		$results['_errors'][]=$arr['soapEnvelope']['soapBody']['soapFault']['faultstring'];
    	$results['xpost']=$post;
    	ksort($results);
    	return $results;
	}
